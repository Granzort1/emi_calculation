...
def calculate_emissions(inputdata, VKT, V, T, ta_min, ta_rise, P_4N, sL=0.06):
    ...
    for pollutant in pollutants:
        if results[pollutant]:
            ...
            if pollutant == 'TSP':
                ...
                output_file = f"C:/dense_traffic_emi/output/resuspension_dust_{pollutant}_4n.xlsx"
                ...
                if results[base_pollutant] and results[resuspension_pollutant]:
                    ...
                    resuspension_df.columns = [
                        ...
                        if col != 'DateTime' and col != f'Total_{resuspension_pollutant}'
                        else col
                        for col in resuspension_df.columns
                    ]

                    # DateTime 열을 기준으로 데이터프레임 병합
                    merged_df = pd.merge(base_df, resuspension_df.drop(f'Total_{resuspension_pollutant}', axis=1),
                                         on='DateTime')
                    ...
                    if f'Total_{base_pollutant}' in merged_df.columns:
                        merged_df = merged_df.drop(f'Total_{base_pollutant}', axis=1)
                    ...
                    # 결과 저장
                    output_file = f"C:/dense_traffic_emi/output/emission_results_{base_pollutant}_4n.xlsx"
                    merged_df.to_excel(output_file, index=False)

    # 배출계수를 찾지 못한 조합들을 엑셀 파일로 저장
    if missing_emission_factors:
        missing_df = pd.DataFrame(list(missing_emission_factors))
        missing_output_file = "C:/dense_traffic_emi/output/missing_emission_factors.xlsx"
...
if __name__ == '__main__':
    ...
    calculate_emissions(inputdata, VKT, V, T, ta_min=ta_min, ta_rise=ta_rise, P_4N=P_4N, sL=sL)
